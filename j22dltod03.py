...
class jlrn:
    ...
    jeleclist = range(16)
    jsize = 19
    jntms = [[0,16], [16,100000]]

def jget_data_train():
    fin = open('./inputs/j22lstm03a5_%s_%s_080.txt' % (jlrn.pat, jlrn.epoch), 'r')
    cnd = dict()
    cld = dict()
...
def jget_data_test():
    fin = open('./inputs/j22lstm03a5_%s_%s_80100.txt' % (jlrn.pat, jlrn.epoch), 'r')
    cnd = dict()
    cld = dict()
    for i in range(len(jlrn.jntms)):
        cnd[i] = 0
...
def jrun():
    ...
    print("Saving", './cmodels/j22dltod033_%s_%s.h5' % (jlrn.pat, jlrn.epoch))
    jlrn.model.save('./cmodels/j22dltod033_%s_%s.h5' % (jlrn.pat, jlrn.epoch))
    ...
    print(jlrn.pat, jlrn.epoch, 'auc hanleyci', a, ci1, ci2)
    tsum = np.sum(jlrn.ydata[:,0])
    fout = open('./sstables/j22dltod033_%s_%s.h5' % (jlrn.pat, jlrn.epoch), 'w')
    for thres in range(10000,-1,-1):
        jsen = np.sum(jlrn.ydata[alist>=thres/10000,0])/tsum
        jsel = np.sum(alist>=thres/10000)/alist.shape[0]
        jtp = np.sum(jlrn.ydata[alist>=thres/10000,0])
        jfp = np.sum(jlrn.ydata[alist>=thres/10000,1])
        jfn = np.sum(jlrn.ydata[alist<thres/10000,0])
        jtn = np.sum(jlrn.ydata[alist<thres/10000,1])
        #cval = np.sum(jlrn.ydata[alist>=thres/100,0])
        print(jlrn.pat, jlrn.epoch, 'thres', thres, jsel, jsen, (jtp/(jtp+jfn)), (jfp/(jfp+jtn)))
        fout.write('%s %s thres %d %g %g %g %g\n' % (jlrn.pat, jlrn.epoch, thres, jsel, jsen, (jtp/(jtp+jfn)), (jfp/(jfp+jtn))))
    return
