...
def jget_data_train():
    fin = open('./inputs/j22lstm01a5_%s_%s_080.txt' % (jlrn.pat, jlrn.epoch), 'r')
    cnd = dict()
    cld = dict()
...
def jget_data_test():
    fin = open('./inputs/j22lstm01a5_%s_%s_80100.txt' % (jlrn.pat, jlrn.epoch), 'r')
    cnd = dict()
    cld = dict()
    for i in range(len(jlrn.jntms)):
        cnd[i] = 0
...
def jrun():
    ...
    print("Saving", './cmodels/j22dltod1001_%s_%s.h5' % (jlrn.pat, jlrn.epoch))
    jlrn.model.save('./cmodels/j22dltod1001_%s_%s.h5' % (jlrn.pat, jlrn.epoch))
    ...
    print(jlrn.pat, jlrn.epoch, 'auc hanleyci 1001', a, ci1, ci2)
    # tsum = np.sum(jlrn.ydata[:,0])
    # fout = open('./sstables/j22dltod07_%s_%s.h5' % (jlrn.pat, jlrn.epoch), 'w')
