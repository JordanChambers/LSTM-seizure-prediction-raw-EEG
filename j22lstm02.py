...
def jrun():
    ...
    jsep = 0
    if sys.argv[2] != '0':
        jlrn.model = tf.keras.models.load_model('./models/j22lstm02_%s_%s.h5' % (sys.argv[1], sys.argv[2]))
    ...
    for jepoch in range(jsep,101):
        ...
        print(jepoch, len(jlrn.cflist), tot[1]/tot[0], tot[2]/tot[0])
        sys.stdout.flush()
        print("Saving", './models/j22lstm02_%s_%d.h5' % (sys.argv[1], jepoch))
        jlrn.model.save('./models/j22lstm02_%s_%d.h5' % (sys.argv[1], jepoch))
    return
